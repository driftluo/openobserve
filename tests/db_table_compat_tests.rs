...
//! Database Table Compatibility Tests
//!
//! This test module verifies table operations compatibility across different
//! database backends using real mock data from `tests/compatible-datasets/`.
//!
//! # What is `prefix`?
//!
//! The `prefix` parameter is used for **test data isolation**:
//! - All test data is created with this prefix (e.g., `test_mysql_user@example.com`)
//! - Prevents conflicts with real data or other test runs
//! - Makes cleanup easy by identifying test data
//!
//! # Running Tests
//!
//! **IMPORTANT**: These tests must be run with `--test-threads=1` because:
//! - Multiple tests operate on the same database tables (e.g., templates)
//! - `test_mysql_all_table_compat` runs all table tests sequentially
//! - Individual tests like `test_mysql_templates_compat` may conflict with it
//! - The global static connection pool architecture prevents per-test database isolation
//!
//! ```bash
//! # Run all tests with MySQL backend (requires db-mysql-tests feature)
//! ZO_TEST_MYSQL_DSN="mysql://root:password@127.0.0.1:3306/openobserve_test" \
//! cargo test --package openobserve --test db_table_compat_tests --features db-mysql-tests -- --test-threads=1 --nocapture
//!
//! # Run a single test (--test-threads=1 is optional for single tests)
//! ZO_TEST_MYSQL_DSN="mysql://root:password@127.0.0.1:3306/openobserve_test" \
//! cargo test --test db_table_compat_tests --features db-mysql-tests test_mysql_organizations_compat -- --nocapture
//! ```
...
/// Test organizations table compatibility with MySQL
...
/// Test users table compatibility with MySQL
...
/// Test templates table compatibility with MySQL
...
/// Test short_urls table compatibility with MySQL
...
/// Test sessions table compatibility with MySQL
...
/// Test system_prompts table compatibility with MySQL
...
/// Test distinct_value_fields table compatibility with MySQL
...
/// Test org_users table compatibility with MySQL
...
/// Test folders table compatibility with MySQL
...
/// Test destinations table compatibility with MySQL
...
/// Test alerts table compatibility with MySQL
...
/// Test dashboards table compatibility with MySQL
...
/// Test file_list table compatibility with MySQL
...
/// Test stream_stats table compatibility with MySQL
...
/// Test file_list_deleted table compatibility with MySQL
...
/// Test file_list_deleted concurrent query_deleted with distributed lock (MySQL GET_LOCK)
...
/// Test file_list_jobs table compatibility with MySQL
...
/// Test file_list_jobs concurrent get_pending_jobs with distributed lock (MySQL GET_LOCK)
...
/// Test file_list_jobs concurrent get_pending_dump_jobs with distributed lock (MySQL GET_LOCK)
...
/// Test scheduled_jobs table compatibility with MySQL
...
/// Test scheduled_jobs concurrent pull with distributed lock (MySQL GET_LOCK)
...
/// Test cipher_keys table compatibility with MySQL
...
/// Test system_settings table compatibility with MySQL
...
/// Test action_scripts table compatibility with MySQL
...
/// Test enrichment_tables table compatibility with MySQL
...
/// Test enrichment_table_urls table compatibility with MySQL
...
/// Test rate_limit_rules table compatibility with MySQL
...
/// Test re_patterns table compatibility with MySQL
...
/// Test re_pattern_stream_map table compatibility with MySQL
...
/// Test service_streams table compatibility with MySQL
...
/// Test service_streams_dimensions table compatibility with MySQL
...
/// Test search_queue table compatibility with MySQL
...
/// Test compactor_manual_jobs table compatibility with MySQL
...
/// Test alert_incidents table compatibility with MySQL
...
/// Test file_list_dump_stats table compatibility with MySQL
...
/// Test schema_history table compatibility with MySQL
...
/// Test reports table compatibility with MySQL
...
/// Test timed_annotations table compatibility with MySQL
...
/// Test timed_annotation_panels table compatibility with MySQL
...
/// Test search_jobs table compatibility with MySQL
...
/// Test search_job_partitions table compatibility with MySQL
...
/// Test search_job_results table compatibility with MySQL
...
/// Test pipeline table compatibility with MySQL
...
/// Test alert_dedup_state table compatibility with MySQL
...
/// Test file_list_history table compatibility with MySQL
...
/// Test pipeline_last_errors table compatibility with MySQL
...
/// Test alert_incident_alerts table compatibility with MySQL
...
/// Test report_dashboards table compatibility with MySQL
